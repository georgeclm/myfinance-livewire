    <ul class="navbar-nav bg-dark sidebar sidebar-dark accordion" id="accordionSidebar">
        ...
        <!-- Heading -->
        <div class="sidebar-heading" style="font-size : 0.8rem;">
            Financial
        </div>
        ...
        <li class="nav-item ">
            ...
            <div id="collapseUtilities" class="collapse @if (in_array(Route::current()->uri, ['transactions', 'transactions/{id}'])) show @endif" aria-labelledby="headingUtilities"
                ...
                <div class="border-0  py-2 collapse-inner rounded">
                    ...
                    <a class="collapse-item text-white @if (Route::current()->uri == 'transactions') active @endif"
                        href="{{ route('transaction') }}">All</a>
                    <a class="collapse-item text-white @if (strrchr(url()->current(), 'o') == 'ons/1') active @endif"
                        href="{{ route('transaction.detail', 1) }}">Income</a>
                    <a class="collapse-item text-white @if (strrchr(url()->current(), 'o') == 'ons/2') active @endif"
                        href="{{ route('transaction.detail', 2) }}">Spending</a>
                    <a class="collapse-item text-white @if (strrchr(url()->current(), 'o') == 'ons/3') active @endif"
                        href="{{ route('transaction.detail', 3) }}">Transfer</a>
                    <a class="collapse-item text-white @if (strrchr(url()->current(), 'o') == 'ons/4') active @endif"
                        href="{{ route('transaction.detail', 4) }}">Pay Debt</a>
                    <a class="collapse-item text-white @if (strrchr(url()->current(), 'o') == 'ons/5') active @endif"
                        href="{{ route('transaction.detail', 5) }}">Friend Pay
                        Debt</a>
                </div>
            </div>
        </li>
